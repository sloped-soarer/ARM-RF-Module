...
#define byte u8
//---------------------------
byte NRF24L01_ReadRegisterMulti_test(byte reg, byte * data, byte length)
{
// SCK_LOW;
  CS_LO();
    byte status_r = PROTOSPI_xfer(R_REGISTER | (REGISTER_MASK & reg));
...
//---------------------------
byte NRF24L01_WriteRegisterMulti_test(byte reg, byte * data, byte length)
{
// SCK_LOW;
  CS_LO();
    byte status_r = PROTOSPI_xfer(W_REGISTER | ( REGISTER_MASK & reg));
    for (byte i = 0; i < length; i++)
    {
    	PROTOSPI_xfer(data[i]);
    }
